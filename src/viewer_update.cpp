...
void Viewer::update_list(){
    ...
    for ( int iii = 0; iii < annotations.size(); iii++ ) {
        if ( iii < 02 )
            ui->finger_list->addItem( side[0] + " hand - " + fingers[3] + "  finger - Point " + QString::number( iii % 2 )+1);
        else
            ui->finger_list->addItem( side[0] + " hand - " + fingers[4] + "  finger - Point " + QString::number(( iii - 2 ) % 2 )+1);
    }
}
